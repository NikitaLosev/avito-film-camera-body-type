"""LLM-разметка объявлений где справочник kb не сработал (около 89k строк)

Используем синхронный API с тарифом Flex (50% скидка от standard, но та же
задержка как у batch). 25 потоков параллельно, один общий кеш на статичную
часть промпта на 26 часов, atomic save каждые 200 строк, идемпотентный
рестарт - на повторном запуске скрипт читает llm_labels.parquet и
пропускает уже размеченные item_id

На случай долгой очереди - retry с экспоненциальным бэкоффом до 5 раз,
жёсткий cost guard на MAX_USD долларов чтобы не сжечь бюджет на ошибках
"""
...
def load_todo():
    """Берём только те строки где KB не дал метку - на них и пускаем LLM"""
...
def main():
    ...
    todo = load_todo()
    if LLM_LABELS.exists():
        acc = pd.read_parquet(LLM_LABELS)
    ...
    def flush():
        ...
        with _lock:
            ...
            atomic_write_parquet(combined, LLM_LABELS)
            acc = combined
            pending.clear()
    ...
    final = pd.read_parquet(LLM_LABELS)
