...
Этот README про устройство пайплайна: что где лежит, как сырой csv превращается в финальный parquet и какие скрипты за это отвечают. Подробная история разметки лежит в `reflection.md`.
...
В `labels_final.parquet` хранится итоговая метка и метки от каждого источника. Поэтому можно понять, откуда взялся класс, без пересборки всего пайплайна
...
Все 12 файлов лежат в `pipeline/`, пронумерованы по порядку запуска. Каждый стейдж можно запускать повторно: на тех же входах он даёт тот же файл. Длинные стейджи (06 и 07, где работает LLM на десятках тысяч строк) пишут parquet через `.tmp` + `os.replace`, поэтому Ctrl+C не побьёт файл, и при следующем запуске они продолжат с того места где остановились
...
Эти скрипты не входят в основной прогон, но я ими пользовался при разметке:
...
| `kb_overridden` | справочник нашёл модель, но LLM по фото с уверенностью >= 0.85 сказала что это не валидная одиночная плёночная камера -> метка демоутится в `other_unknown` |
